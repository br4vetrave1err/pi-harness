...
# Dashboard landing: docker exec -it pi-personal-agent bash -> CLI dashboard (not directly pi)
# Left 1/4: previous conversations (workspace) with agent tag, Middle 3/4: running agent windows
# Selecting a window lands in pi-vCLI (pi --session). Bypass with PI_NO_AUTO=1 or bash --noprofile
RUN cat > /root/.bashrc.pi-dashboard << 'BASHRC'
# pi-dashboard auto-attach
if [[ $- == *i* ]] && [ -t 0 ] && [ -z "$PI_NO_AUTO" ] && [ -z "$PI_MANUAL_BASH" ]; then
  if command -v pi >/dev/null 2>&1; then
    if [ $# -eq 0 ]; then
      # Prefer dashboard if available, fallback to pi-main
